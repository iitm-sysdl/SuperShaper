    ...
    with torch.no_grad():
        ...
        for ind in range(num_hidden_layers):
            ...
            for keys in keys_to_permute:
                ...
                if 'attention.output.dense' in key or 'attention.output.LayerNorm' in key or 'intermediate.dense' in key:
                ...
                if 'output_bottleneck' in key:
                    if ind < num_hidden_layers-1:
                        module_next = attrgetter(f"bert.encoder.layer.{ind+1}.attention.self.query")(model)

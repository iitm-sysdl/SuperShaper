...
class CustomMultiheadAttentionSuper(nn.Module):
    ...
    def __init__(
        ...
        num_heads,
        is_encoder,
        super_kdim=None,  # TODO: add super_qdim
        super_vdim=None,
        dropout=0.0,
        bias=True,
        add_bias_kv=False,
        ...
        # TODO: remove this variable and allow elastications for d_v and
        # d_k( d_k and d_q should be the same as they are matrix multipled
        # in self attention equation)
        ...
        assert (
            self.head_dim * num_heads == self.qkv_dim
        ), "qkv must be divisible by num_heads"
        ...
        if self.qkv_same_dim:
            self.in_proj_weight = Parameter(
                torch.Tensor(3 * self.qkv_dim, self.super_q_embed_dim)
            )
        else:
            # TODO: we will use this with differnet tensor shapes
            # it will have different d_v, d_k
            # change the tensor shapes accordingly

            self.k_proj_weight = Parameter(
                # (self.qkv_dim, self.super_kv_embed_dim) -> (hdk, d_model)
                torch.Tensor(self.qkv_dim, self.super_kv_embed_dim)
            )
            self.v_proj_weight = Parameter(
                # (self.qkv_dim, self.super_kv_embed_dim) -> (hdv, d_model)
                torch.Tensor(self.qkv_dim, self.super_kv_embed_dim)
            )
            self.q_proj_weight = Parameter(
                # (self.qkv_dim, self.super_kv_embed_dim) -> (hdk, d_model)
                torch.Tensor(self.qkv_dim, self.super_q_embed_dim)
            )
        ...
        # this out_dim should be hdv in our case
        if out_dim is None:
        ...
        # intialize the weights for supertransformer
    ...
    def calc_sampled_param_num(self):
        # TODO: change accordding to new d_V and d_q
        ...
        in_proj_v_weight_numel = in_proj_k_weight_numel = (
            self.sample_kv_embed_dim * self.qkv_dim
        )
        in_proj_bias_numel = self.in_proj_bias.numel()

        # does not count in the output proj because it will be counted in CustomLinear layer
    ...
    def set_sample_config(
        self, sample_q_embed_dim, sample_attention_heads, sample_kv_embed_dim=None
    ):
        ...
        # TODO: sampled_head_dim = sampled_dv(setting d_v as a elastic)
        self.head_dim = self.qkv_dim // self.num_heads
        assert (
            self.head_dim * self.num_heads == self.qkv_dim
        ), "qkv_dim must be divisible by sampled num_heads"
        # sampled_dk instaed of self.head_dim(sampled_head_dim)
        self.scaling = self.head_dim ** -0.5
        # self.qkv_dim -> hdv
        # set_sample_config/slicing on q,k,v is not done in this function
        # This is happening in forward
    ...
    def reset_parameters(self):
        # TODO: change according the dimensions d_k, d_v
        if self.qkv_same_dim:
    ...
    def forward(
        ...
        incremental_state=None,  # this is only used for inference for incremental decoding
        # https://github.com/pytorch/fairseq/issues/252
        need_weights=True,
        static_kv=False,
        attn_mask=None,
    ):
    ...
    def in_proj_q(self, query):
        if self.qkv_same_dim:
            return self._in_proj(
                query, end=self.qkv_dim, sample_dim=self.sample_q_embed_dim
            )
        else:
            ...
            return F.linear(
                query, self.q_proj_weight[..., : self.sample_q_embed_dim], bias
            )
    ...
    def in_proj_v(self, value):
        if self.qkv_same_dim:
            return self._in_proj(
                value, start=2 * self.qkv_dim, sample_dim=self.sample_kv_embed_dim
            )
        else:

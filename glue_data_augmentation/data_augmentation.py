...
# script taken and adapated from https://github.com/huawei-noah/Pretrained-Language-Model/blob/master/TinyBERT/data_augmentation.py to support cased models
...
class DataAugmentor(object):
    ...
    def _masked_language_model(self, sent, word_pieces, mask_id):
        ...
        word_candidates = torch.argsort(predictions[0, mask_id], descending=True)[
            : self.M
        ].tolist()
...
def main():
    ...
    parser.add_argument(
        "--glue_dir", default=None, type=str, required=True, help="GLUE data dir"
    )
    parser.add_argument(
        "--task_name",
        default=None,
    ...
    parser.add_argument(
        "--N", default=30, type=int, help="How many times is the corpus expanded?"
    )
    parser.add_argument(
        "--M",
        default=15,
    ...
    data_augmentor = DataAugmentor(
        model, tokenizer, emb_norm, vocab, ids_to_tokens, args.M, args.N, args.p
    )
